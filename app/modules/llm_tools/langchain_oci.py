...
class OCIGenAIBase(BaseModel, ABC):
    ...
    """Authentication type, could be API_KEY, SECURITY_TOKEN, INSTANCE_PRINCIPLE, RESOURCE_PRINCIPLE
    ...
    provider: str = None
    """Provider name of the model. Default to None, will try to be derived from the model_id
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            ...
            values["service_models"] = oci.generative_ai_inference.models
    ...
    def _get_provider(self) -> str:
        ...
        if provider not in VALID_PROVIDERS:
            raise ValueError(
                ...
                "Please explicitly pass in the supported provider when using custom endpoint"
            )
        return provider
...
class OCIGenAI(LLM, OCIGenAIBase):
    ...
    access the OCI Generative AI service. If a specific config profile is used, you must pass
    ...
    def _prepare_invocation_object(self, prompt: str, stop: Optional[List[str]], kwargs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        oci_llm_request_mapping = {
            "cohere": self.service_models.CohereLlmInferenceRequest,
            "meta": self.service_models.LlamaLlmInferenceRequest,
        }
        ...
        if self.model_id.startswith(CUSTOM_ENDPOINT_PREFIX):
            serving_mode = self.service_models.DedicatedServingMode(endpoint_id=self.model_id)
        else:
            serving_mode = self.service_models.OnDemandServingMode(model_id=self.model_id)
        
        inference_params = {**_model_kwargs, **kwargs}
        inference_params["prompt"] = prompt
        inference_params["is_stream"] = self.is_stream

        invocation_obj = self.service_models.GenerateTextDetails(
